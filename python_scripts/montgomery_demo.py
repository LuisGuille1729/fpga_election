...
@timer
def run_montgomery_exponentiation_no_divider(val: int, exp: int, constants: dict):
    N = constants["N"]
    R = constants["R"]
    P = constants["P"]
    k = constants["k"] 
    
    _, residue_class = non_restor_div((val * R), N) 
    # equivalent to (val * R) % N 
    # if we actually want this, why not
    # Redc(val*R*R) which is equal to val*R*R*R^-1 % N = val*R % N
    # residue_class = Redc(val*R*R, constants)
    
    print(Redc(Redc(val*R, constants)*R, constants))
    print(val*R % N)
    
    # residue_class = Redc(val*R, constants)
    # print(residue_class)
...
if __name__ == "__main__":
    ...
    result2 = run_montgomery_exponentiation_no_divider(val, exp, constants)
