...
async def NextValidMultiplierOut(dut, k_chunk_idx, N_chunk_idx, print_details=False):
    ...
    while dut.multiplier_valid_out.value != 1:
        assert ctr < MAX_CYCLES_BEFORE_DEADLOCK_DETECTED, "Never received an expected multiplier_valid_out"
        if print_details:
            if (ctr % 10000) == 0:
                print("In here 1!")
...
async def verify_multiplier_outputs(dut, previous_montgomery_output, k_chunk_idx, N_chunk_idx, print_details=False):
    ...
    if print_details:
        ...
        print("\nFull expected multiplier out:")
        for i in range(NUM_BLOCKS_INTO_REDUCER):
            block_to_print = (expected_multiplier_output >> i*REGISTER_SIZE) & REGISTER_SIZE_ALL_ONES
            print(f"\tBlock {i+1}: {hex(block_to_print)}")
