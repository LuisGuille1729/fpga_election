...
def dispatch_blocks(value, num_blocks, infinite=True):
    while True:
        ...
        for _ in range(num_blocks):
            yield val & (2**REGISTER_SIZE_IN-1)
            val = val >> REGISTER_SIZE_IN
...
async def test_mem(dut, num_1, num_2):
    ...
    dut.valid_in.value = 0
    for _ in range(BITS_IN_NUM//REGISTER_SIZE_IN):
...
async def test_product(dut, num_1, num_2):
    ...
    cycles = BITS_IN_NUM//REGISTER_SIZE_IN
...
async def test_nums(dut,num_1,num_2):
    ...
    clk_cys = 0
    for i in range(BITS_IN_NUM//REGISTER_SIZE_IN):
        n_i = num_1//(2**(REGISTER_SIZE_IN*i))% (2**REGISTER_SIZE_IN)
        m_i = num_2//(2**(REGISTER_SIZE_IN*i))% (2**REGISTER_SIZE_IN)
        await send_num(dut,n_i,m_i)
        clk_cys += 1
        if (dut.valid_out.value == 1):
            # print("got", int(dut.data_out.value))
            running_prod += (int(dut.data_out.value))*(2**(REGISTER_SIZE_IN*nums_received))
            nums_received += 1
    
    while (nums_received != 2*BITS_IN_NUM//REGISTER_SIZE_IN ):
        await ClockCycles(dut.clk_in,1)
        clk_cys += 1
        # print("got", int(dut.data_out.value))
        if (dut.valid_out.value == 1):
...
BLOCKS_INPUT = BITS_IN_NUM//REGISTER_SIZE_IN # 128
BLOCKS_OUTPUT = BITS_IN_OUT//REGISTER_SIZE_IN # 256
def test_runner():
    ...
    runner.build(
        sources=sources,
        hdl_toplevel="fsm_multiplier_parallel",
        always=True,
    ...
    runner.test(
        hdl_toplevel="fsm_multiplier_parallel",
        test_module="test_fsm_multiplier_parallel",
